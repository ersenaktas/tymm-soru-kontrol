...
> **Geçici sunucu testi:** Kullanıcının açık talebiyle mevcut `storage_state.json`, bu sunucu kopyasında `server_auth/storage_state.json` altına gömülmüştür ve Docker imajı `/app/server_auth/storage_state.json` yolunu kullanır. Bu yöntem yalnız pilot test içindir. Test sonunda gömülü dosya silinmeli, Google oturumu yenilenmeli ve aşağıdaki Secret File yöntemine dönülmelidir.
